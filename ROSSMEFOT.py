while True:
    try:
        ...
        if platform.system() == "Windows":
            ...
            enter = pystyle.Colorate.Horizontal(pystyle.Colors.white_to_red, ('hi'))
            pystyle.Anime.Fade(
            pystyle.Center.Center('''йоу'''), pystyle.Colors.white_to_red, pystyle.Colorate.Vertical, enter=True)
        def Main():
            if platform.system() == "Windows":
                os.system("cls")
                pystyle.Write.Print(pystyle.Center.XCenter('''бебра'''), pystyle.Colors.white_to_red, interval = 0.0005)
            else:
                os.system("clear")
...
└─────────────────────────────────────────────────────────────────────────┘ @kabakui                                                            
 ...
 [11] Мануал по свату       [22] Зал славы                      
                  ...
                  Переработано [https://t.me/kabakui]                                           
                         [23] Выход                          
...
Отправили письмо - следуйте этим же шагам в правильной последовательности. Выйдите из Tor, Linux, а затем из VPN.\n''', pystyle.Colors.white_to_red, interval = 0.005)
            ...
            if choice == "16":
                def dos1():
                    try:
                        ...
                        print(f'\n{colorama.Fore.white_to_red}[{colorama.Fore.WHITE}!{colorama.Fore.white_to_red}]{colorama.Fore.MAGENTA} Произошла ошибка! Данные не подходят!')
                dos1()
            if choice == "17":
                pystyle.Write.Print("\n[?] Выберите страну:\n", pystyle.Colors.white_to_red, interval=0.005)
            ...
            if choice == "13":
                pystyle.Write.Print('''\nПровоцируем жертву на угрозы, оски, буллинг и тд. Дальше пишем на почту DMCA@telegram.org. В письме указывем скриншоты булинга, угроз и тд. "Здраствуйте, хочу подать жалобу на (юзер и айди типа).  уважением, пользователь" и все, главное скриншоты - доказательства,но способ уже не рабочий!\n''', pystyle.Colors.white_to_red, interval = 0.005)
            if choice == "1488":
                if platform.system()=='Windows':
